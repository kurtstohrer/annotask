...
- **Wireframe mode** — Freezes the current route into a manipulable image canvas: `wireframe:capture` rasterizes the page into per-block html2canvas snapshots (each carrying its `data-annotask-file/-line` source anchor), the canvas supports drag (snap/align guides; Alt disables), 8-handle resize, marquee/shift-click multi-select with group move, arrow-key nudging (Shift = 10px), soft-delete/undelete, duplicate, notes, double-click explode-to-children (re-captures one block a level deeper with per-child anchors), palette-drop component snapshots through a generate panel (pick → settings → optional data binding → honest preview snapshot on the app-true surface → place; gear button reconfigures in place), and drawn sections (a labeled box, optionally with a markdown spec and a data-source binding picked from the REAL catalog with shape drill-down). State persists per-route in `wireframe.json` (PNGs in `.annotask/wireframe-snapshots/`, orphans GC'd at boot) and survives F5; capturing under a viewport preset wireframes that device width. "Implement this wireframe" diffs the sketch against the original capture into anchored `wireframe_direction` entries (relations like "now above the filters" are the contract; pixels are hints), composes a labeled before/after screenshot, and runs the embedded agent through the apply loop — accept/undo/discard ride the file-snapshot engine, and in git projects agent-CREATED files are netted into the undo batch (hash-guarded deletion). The live iframe stays mounted under the canvas, so exit is lossless. **NOTE (current release): data-source binding is gated OFF via `src/shell/wireframeFeatures.ts` — its code paths and persisted bindings remain (render read-only), but the UI entry points are hidden and new tasks won't carry bindings. Double-click explode-to-children is ON: MFE-aware, with a geometric fallback for anchorless blocks.**
