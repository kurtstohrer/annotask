...
- **Wireframe mode** — Freezes the current route into a manipulable image canvas: `wireframe:capture` rasterizes the page into per-block html2canvas snapshots (each carrying its `data-annotask-file/-line` source anchor), the canvas supports drag/resize/delete/duplicate/notes plus palette-drop component snapshots and drawn placeholders, state persists per-route in `wireframe.json` (PNGs in `.annotask/wireframe-snapshots/`) and survives F5. "Implement this wireframe" diffs the sketch against the original capture into anchored `wireframe_direction` entries (relations like "now above the filters" are the contract; pixels are hints), composes a labeled before/after screenshot, and runs the embedded agent through the apply loop — accept/undo/discard ride the file-snapshot engine. The live iframe stays mounted under the canvas, so exit is lossless
