...
### Added
- **Universal proxy mode — the design tool for *any* running site.** `annotask serve --target <url>` hosts the full Annotask server on its own port and reverse-proxies every request to the target app, injecting the bridge into proxied HTML on the way through. That gives production builds, CDN/no-build pages, non-Node backends (Rails/Django/PHP), and remote staging the annotation/screenshot/a11y/error/perf/wireframe surface with **zero build integration**. Everything shares one origin, the target's CSP and `X-Frame-Options` are stripped on proxied responses, absolute same-origin redirects are rewritten back onto the proxy, and non-`/__annotask` WebSocket upgrades tunnel through so dev-server HMR keeps working. Source anchors come from whatever the page offers — native framework debug metadata where present, honest empty anchors plus DOM fingerprints everywhere else.
- **DOM fingerprints + a fingerprint resolver for anchorless blocks.** Blocks with no `data-annotask-*` source anchor (un-instrumented library `dist` DOM, proxied third-party sites) now carry `{ selector, textHead, htmlHead }` fingerprints end-to-end into the wireframe directions. A new `GET /api/resolve-fingerprint` endpoint (plus `annotask_resolve_fingerprint` / `annotask resolve-fingerprint`) turns that DOM evidence into ranked candidate source locations via a uniqueness-weighted grep, and capture-time auto-resolve stamps a confident sole candidate straight onto the block. Native debug anchors are read where frameworks expose them (Svelte `__svelte_meta`, React fiber `_debugSource`, htmx/Turbo fragment URLs).
- **Shadow-DOM capture.** The wireframe walker descends through open shadow roots (with slot resolution), treats an `<iframe>` as an opaque boundary it won't decompose, and honestly stamps closed-shadow hosts `shadow-opaque` instead of guessing their interior.
- **Render-once capture.** The capture path rasterizes one master canvas and crops per-block instead of replaying up to 25 full-document renders, with a master→scale-1→bounded-legacy fallback ladder so a slow page degrades with a visible warning rather than a multi-minute stall. Capture-time fingerprint auto-resolve and fragment provenance ride the same pass.
- **Session resume across all four local-CLI agents.** claude / codex / opencode / copilot now resume a prior session by id and send only the delta message (with a cold-spawn holdback that silently retries a stale session), gated by a per-provider capability probe and a kill-switch setting. claude cold spawns use `--append-system-prompt` so the skill prompt becomes a cacheable system segment; codex takes the prompt on stdin. The headless auto-run driver **chains** compatible queued tasks onto one resumed session. Together these cut the per-turn token overhead of the embedded-agent apply loop.

### Fixed
- **Wireframe correctness (critical-review P0/P1).** A late in-flight component preview can no longer mutate a locked ("building") sketch; fingerprint auto-resolve honors the resolver's truncated-scan (`capped`) flag so a coincidental sole candidate on a large monorepo isn't trusted for codegen; the upload-route body cap was raised so the documented 4 MB PNG limit is the real boundary; a transient reload failure keeps the last-good wireframe doc instead of blanking every consumer; explode claims its capture lock before the first await, falls back to rect identity for re-minted element ids, and refuses while building; and the apply batch is snapshotted **before** the task is minted so no broadcast can race the safety net.
- **Screenshots and the before/after composite no longer silently vanish on a tainted image.** Every `html2canvas` call site now renders with `allowTaint: false`, so a single non-CORS cross-origin image rasterizes blank instead of throwing `toDataURL` and discarding the whole capture. Blocks overlapping such an image get an `extImages` stamp and an "external images" pill, so a blank region is a labeled cause, not a silent white hole.
- **Stale-viewport reuse.** Re-entering a route whose persisted canvas was captured at a very different viewport width now shows a dismissible mismatch banner (Recapture / Keep sketch) instead of a silently mis-sized sketch, and the canvas is horizontally centered on its content rather than stuck scrolled left.
- **Micro-frontend capture and apply.** Loop-rendered blocks carry a shared-anchor caution instead of a single ambiguous anchor, an exploded parent and its children de-dupe on move, monorepo undo coverage runs at the git toplevel, and MFE block anchors attribute down to their own package rather than the host shell.
- **~50 component- and data-detection defects.** Highlights: the lucide barrel scan no longer OOM-crashes the dev server; every scanned component now records how its props were extracted (`cem`/`dts`/`source`/`name-only`) so unknown props are labeled honestly; monorepo path resolution retries against the workspace root, fixing details/shape/binding-classify across every workspace; and the data scanner gained dep-gated library confirmation, axios/`$fetch`/htmx detection, and endpoint provenance.
- **Bridge injection corruption.** A `$'` / `$&` / `__reactFiber$` sequence in the bridge source no longer expands through `String.replace` and truncates the injected script — all injectors use a function replacer, now regression-tested along with the proxy's decode and header-strip transforms.
- **Hardened the task create boundary.** `POST /tasks` and `annotask_create_task` now structurally validate a `wireframe_apply` task's `context`, so a same-origin caller can't mint one carrying a malformed context the apply agent would trust. Server-minted applies are unaffected (they bypass the HTTP/MCP boundary), and every other task type keeps its free-form context.

### Changed
- **Wireframe surface is now *expanded*, not reduced.** The 0.4.0 "wireframe surface reduced" note is superseded: capture now reaches any site (proxy mode), open shadow DOM, and render-once. **Data binding remains gated off** (`WIREFRAME_DATA_BINDING_ENABLED = false`) — the picker and any persisted bindings render read-only and new tasks carry no binding — while the shape-confidence honesty ladder is finished. This release lands the binding-honesty *groundwork* only; the flip is a later release.
